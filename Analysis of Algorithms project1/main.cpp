...
using namespace std;
struct myset{ // the structs contain all information for each line in the file.
    string country;
    string item_type;
    string order_id;
    string units_sold;
    string total_profit; // use string type in order to write the file properly.
    float float_total; // use float version in order to compare number value.
};

vector<struct myset> vec; //structs,myset, will be kept in vector.


bool larger(myset first, myset second){ // first sort by the country names if names are same , sort by total profits
    if(first.country > second.country){ // first compare country_names
        return true;
    }
        ...
        if(first.float_total <= second.float_total) // if country names are same, compare total_profits
            return true;
        return false;
    }
}
...
bool larger2(myset first, myset second){ // Sort the data according to the country names
...
void quicksort(int first, int last){ // Quicksort Algoritm Function
    ...
    while(i < j){ //until i and j are the same or j>i
        while(larger(vec[first],vec[i]) and i < last) // as long as the first vector is bigger than vector[i], increase i
            i  = i + 1;
        while(larger(vec[j],vec[first]) and j > first+1) // as long as the vec[j] is bigger than first vector , decrease j
            j = j - 1;
    ...
    for(int i = first; i < last; i++){
        if(vec[i+1].country > vec[first].country or (vec[i+1].country == vec[first].country and vec[i+1].float_total < vec[first].float_total ) ){
            answer = i;
            break;
        }
    }
...
void write_to_file(int N,float time){ // writes all the data to the file
    ofstream myfile;
    myfile.open("sorted.txt"); // opens the file
