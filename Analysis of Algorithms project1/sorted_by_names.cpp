...
// read from sorted_by_profits.txt and write to sorted_by_names.txt
...
void quicksort(int first, int last){ // Quicksort Algoritm Function
    ...
    while(i < j){ //until i and j are the same or j>i
        while(larger2(vec[first],vec[i]) and i < last) // as long as the first vector is bigger than vector[i], increase i
            i  = i + 1;
        while(larger2(vec[j],vec[first]) and j > first+1) // as long as the vec[j] is bigger than first vector , decrease j
            j = j - 1;
    ...
    for(int i = first; i < last; i++){
        if(vec[i+1].country > vec[first].country ){
            answer = i;
            break;
        }
    }
...
void write_to_file(int N,float time){ // writes all the data to the file
    ofstream myfile;
    myfile.open("sorted_by_names.txt"); // opens the file
...
int main(int argc, char *argv[]){
    float y;
    ifstream file;
    file.open("sorted_by_profits.txt"); // opens the file to read.
